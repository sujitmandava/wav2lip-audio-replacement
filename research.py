...
class LipSyncPipeline:
    ...
    def extract_frames_and_audio(self, video_path):
        """Extract frames with better temporal resolution for lip sync"""
        try:
            ...
            # For better lip sync, process more frames
            # Process every 2nd frame for videos > 30fps, every frame for slower videos
            if fps > 30:
                frame_skip = 2
            else:
                frame_skip = 1
            
            logger.info(f"Video FPS: {fps}, using frame_skip: {frame_skip}")
            ...
            while True:
                ...
                # Process frames at higher temporal resolution for better sync
    ...
    def get_mel_chunks(self, audio, sr, fps):
        """Produce mel chunks with proper timing alignment for video frames"""
        try:
            # Wav2Lip specific parameters
            n_mels = 80
            n_fft = 800
            hop_length = 200  # This gives us 80 mel frames per second (16000/200)
            win_length = 800
            fmin = 55
            fmax = 7600
            ...
            # Wav2Lip uses 16 consecutive mel frames (0.2 seconds of audio)
            mel_step_size = 16
            mel_fps = sr / hop_length  # Should be 80 Hz
            
            logger.info(f"Mel spectrogram: {mel_spec.shape}, Mel FPS: {mel_fps}, Video FPS: {fps}")
            
            # Calculate how many mel frames correspond to each video frame
            mel_frames_per_video_frame = mel_fps / fps
            
            chunks = []
            n_mel_frames = mel_spec.shape[1]
            
            # Ensure we have enough mel frames
            if n_mel_frames < mel_step_size:
                pad_amt = mel_step_size - n_mel_frames
                mel_spec = np.pad(mel_spec, ((0, 0), (0, pad_amt)), mode='edge')
                n_mel_frames = mel_spec.shape[1]

            # Create chunks with proper temporal alignment
            # Each chunk represents ~0.2 seconds of audio
            chunk_duration_frames = mel_step_size / mel_fps  # ~0.2 seconds
            video_frames_per_chunk = int(chunk_duration_frames * fps)  # video frames per chunk
            
            # Generate overlapping chunks for smoother sync
            step = max(1, mel_step_size // 2)  # 50% overlap for smoother transitions
            
            for i in range(0, n_mel_frames - mel_step_size + 1, step):
            ...
            logger.info(f"Created {len(chunks)} mel chunks with 50% overlap for better sync")
            return chunks
    ...
    def process_keyframes_with_progress(self, frame_data, audio, sr, job_id):
        """Process keyframes with precise audio-video synchronization"""
        try:
            ...
            mel_chunks = self.get_mel_chunks(audio, sr, fps)
            ...
            logger.info(f"Frame skip: {frame_skip}, FPS: {fps}")
            ...
            mel_fps = sr / 200  # 200 is hop_length, gives us 80 Hz
            video_duration = len(frame_data['all_frames']) / fps
            mel_duration = len(audio) / sr
            
            logger.info(f"Video duration: {video_duration:.2f}s, Audio duration: {mel_duration:.2f}s")
            
            for i in range(total_keyframes):
                # Update progress
                jobs[job_id]['progress'] = (i + 1) / total_keyframes * 70
                jobs[job_id]['message'] = f"Syncing keyframe {i+1}/{total_keyframes}"

                frame = keyframes[i]
                keyframe_idx = keyframe_indices[i]
                
                # Calculate precise timing for this frame
                frame_time = keyframe_idx / fps  # Time in seconds for this frame
                
                # Find corresponding mel chunk based on precise timing
                # Each mel chunk represents ~0.2 seconds, with 50% overlap
                chunk_duration = 16 / mel_fps  # ~0.2 seconds per chunk
                overlap_step = chunk_duration / 2  # 50% overlap
                
                # Find the mel chunk that best matches this frame's timing
                target_chunk_idx = int(frame_time / overlap_step)
                target_chunk_idx = min(target_chunk_idx, len(mel_chunks) - 1)
                
                mel_chunk = mel_chunks[target_chunk_idx]
                
                logger.debug(f"Frame {i}: time={frame_time:.2f}s, using mel chunk {target_chunk_idx}/{len(mel_chunks)}")
                ...
                try:
                    ...
                    with torch.no_grad():
                        ...
                        if i == 0:
                            logger.info(f"First keyframe synced: frame_time={frame_time:.2f}s, mel_chunk={target_chunk_idx}")
